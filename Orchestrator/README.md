...
is the onboarding path in depth; the three **sample-game repositories** published
alongside UGT — each a game built from a PRD by this Orchestrator, paired with the
UGT integration that tests it — are the fastest thing you can actually run.
...
worked example of each of the three bridges — `escape-room/` (simulation),
`dice/` (browser) and `sokoban/` (custom) — and
`sokoban/integration/` is the fastest orientation available, since it runs
