...
**Status: FULL LADDER COMPLETE 2026-07-07.** tarot-war is a two-player tarot-themed
card-battle game — a turn-based duel with a war/tie-breaker mechanic driven by Major
Arcana card effects, three modes (classic/survival/endless) and three AI difficulties.
This README was already the best-organized of the surveyed games' READMEs; what
follows is a light addendum (LLM-playtest + harness self-audit sections), not a
rewrite. For full resume state see **[HANDOFF.md](HANDOFF.md)**; for the complete
findings log see **[RESULTS.md](RESULTS.md)**. No `archive/` directory exists for
this integration — nothing here was clutter.

Drives the **real** tarot-war React/Vite game (`../tarot-war`, branch `main`) in a
headless browser via `PlaywrightAdapter` (`engine.type: browser`) and the UGT hooks
installed in the game at `src/ugt-hooks.ts` (`window.__GET_STATE__` /
`__SEND_ACTION__` / `__RESET_GAME__(seed)`).
...
Note: no `spike_*.py` or `smoke_*.py` scripts are tracked for this game — the
tracked ladder starts at `verify_round1.py`.
...
# 4. LLM balance-playtest tier (ugt.config.yaml's playtest: block + strategy-guide.md):
ugt playtest --config integrations/tarot-war/ugt.config.yaml \
  --strategy-guide integrations/tarot-war/strategy-guide.md --max-actions 30

# 5. No-server regression check for the R3 determinism predicate:
python3 integrations/tarot-war/determinism_selftest.py
```

No naming-confusion risk with any other integration in this repo — no special
disambiguation banner needed here.
...
- **TW-R8 (major, found in Round 2) — FIXED & VERIFIED LIVE (fixed twice).**
  The World's instant victory set `gamePhase='finished'` mid-pipeline but the
  round KEPT RESOLVING: the opponent's card effect still executed (a
  post-finish Magician would move cards between players), the claim still
  awarded +2, and a tie would have rolled a full war — all after the game was
  over. Observed live twice: first as a post-victory claim log, then (after a
  first fix guarded only the end of the effect phase) as the opponent's
  Hanged Man effect firing post-finish. Fix: resolution stops the moment an
  effect finishes the game — the other card's effect is skipped and the
  interrupted round's cards return to their owners' discards (census intact).
...
- **TW-R4 (minor) — CLOSED: root-caused to TW-R2 with high confidence
  ...
  crash path; closed. Update (Round 2): no recurrence — Round 2 played
  endless mode to completion live plus dozens more suite runs; still 1
  occurrence total. TW-R2/TW-R8 both removed crash-classes from the endless
  path. Stays open as a watch-item.

**One open design observation (not a bug):** in classic mode `score` is
*cumulative cards claimed*, while the win comes from opponent
total-exhaustion — so the classic winner can finish with the LOWER cumulative
score (seen live: 178–179). Worth a balance look at whether GameSummary
explains this to players; not flagged as a defect.
...
## LLM playtest tier (L-005, 2026-07-21)

WIRED and smoke-run: `ugt playtest` against ollama `gemma4:26b` completed 30/30
real `play_round` dispatches through `__SEND_ACTION__`, producing
`results/playtest-report.json` with 0 bugs and 0 invariant violations.
`ugt.config.yaml` carries the `playtest:` block and `strategy-guide.md` exists.
A deeper balance verdict — varying AI difficulty, a stronger model / the
Anthropic provider — remains open and is credit-gated; gemma4:26b only ever
dispatched `play_round` for the whole run.

## 2026-07-21 harness self-audit (L-001)

Swept the three ladder scripts for the DDD/Pond vacuous-check class and found
and fixed ONE tester-side defect: the R3 same-seed determinism check was
vacuously true on an empty trajectory (`same_len and divergence is None` is
`True` for empty/empty). Fixed by extracting a `trajectories_match` predicate
that fails closed on empty input, pinned with the committed
`determinism_selftest.py` regression artifact (6/6 cases). No change to the
game itself; live MET counts unaffected.
