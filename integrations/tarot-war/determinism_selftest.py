#!/usr/bin/env python3
"""Committed negative test for the same-seed determinism predicate in verify_round3.
...
Run: python3 integrations/tarot-war/determinism_selftest.py
"""
...
from verify_round3 import trajectories_match  # noqa: E402


def _old_inline_predicate(first: list, second: list) -> bool:
    """The exact boolean the fix replaced (verify_round3.py before L-001):
...
def main() -> int:
    ...
    a = [(1, "resolving", 1, 5, 3), (0, "playing", 1, 5, 3), (1, "resolving", 2, 7, 3)]
    ...
    b = [(1, "resolving", 1, 5, 3), (0, "playing", 1, 5, 3), (1, "resolving", 2, 9, 3)]
    ok, detail = trajectories_match(a, b)  # differs from `a` at index 2
    ...
    if fails:
        print("Tarot-war determinism self-test FAILED:")
        for f in fails:
            print(f"  * {f}")
        return 1
    print("Tarot-war determinism self-test PASSED (6/6 cases).")
    return 0
