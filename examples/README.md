...
| [**sokoban**](sokoban/README.md) | Sokoban Mini — push three levels' worth of crates onto targets | Godot 4 | `custom` (TCP, hand-written adapter) | 89/89 | 18 · 9 · 14 · 15 · 7 | **not built** — the open gap |
...
Every one of those was found for free, on a local model, before a paid call. Which is the whole argument for the staging.

Two of the three games have been through it: escape-room's local pilot now escapes, dice ran both local stages clean and is waiting on a credit decision. Sokoban hasn't, and that's the honest open gap in this example set — it's the only one of the three living inside a game engine, so it's the only place the question "can a model play through a frame loop" gets answered. Preparing it has already paid for itself twice (it's what found the missing R key and the invisible boxes) without a single model call.
