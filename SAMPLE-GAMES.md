# Sample Games — built with the Orchestrator, tested with UGT
...
> **These games are no longer in this repository.** As of 2026-07-27 each is its own repository, published
> alongside UGT rather than bundled inside it. That is deliberate, and this document already said why long
> before the change was made (see the layout sketch below): *the game gets its own repo.* Keeping them under
> `examples/` here taught the opposite of what the method actually is, and it forced three games on anyone
> who only wanted the tester. This file stays because it is the narrative of how the whole method works —
> PRD → task list → orchestrate → harness → ladder → playtest. The per-game links below point at the
> standalone repositories.

| | Game | Stack | Transport | Game suite | Ladder | LLM tier |
|---|---|---|---|---|---|---|
| **dice** *(own repo)* | Dice Duel — allocate six dice between attack and defense, twelve rounds | React + Vite | `browser` (Playwright) | 162/162 | 19 · 9 · 12 · 14 · 13 | local stages done; paid run is a credit decision |
| **escape-room** *(own repo)* | Tiny Escape Room — ten rooms, an eight-link chain of locks | Node.js | `simulation` (stdio) | 104/104 | 30 · 12 · 18 · 56 · 10 | local pilot **escaped**; paid run not started |
| **sokoban** *(own repo)* | Sokoban Mini — push three levels' worth of crates onto targets | Godot 4 | `custom` (TCP, hand-written adapter) | 115/115 | 18 · 11 · 14 · 15 · 7 | local stage done — transport proven, model below the floor; paid run not started |
...
│   ├── _dice                   # The sample games, each its own repo. Leading underscore
│   ├── _escape-room            #   keeps them sorted next to _UGT rather than scattered
│   └── _sokoban                #   through the alphabet.
```

As of 2026-07-27 this is exactly how the sample games are laid out too — the `examples/` line that used to
sit inside `_UGT` is gone, because the advice in the paragraph above applies to them as much as to yours.
